...
class LegalCase(Base, UUIDMixin, TimestampMixin):
    ...
    # Core identifiers
    case_id: Mapped[str] = mapped_column(String(255), unique=True, nullable=False, index=True)
...
class CaseChunk(Base, UUIDMixin, TimestampMixin):
    ...
    chroma_id: Mapped[str | None] = mapped_column(String(255), nullable=True, index=True)
    ...
    def __repr__(self) -> str:
        return f"<CaseChunk id={self.id} case_id={self.case_id} index={self.chunk_index}>"
